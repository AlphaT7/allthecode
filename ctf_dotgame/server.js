    ...
    var methods = {
        ...
        moveFlagRunners1: function(gamename) {
            ...
            for (var dot in gd) {
                if (gd[dot].dottype == 'flagrunner' && gd[dot].live == true) {
                    ...
                    var targetaquired = x == g.flags1[0].x && y == g.flags1[0].y;
                    var targetoffset = x - 7 == g.flags1[0].x && y - 7 == g.flags1[0].y;
        ...
        moveFlagRunners2: function(gamename) {
            ...
            for (var dot in gd) {
                if (gd[dot].dottype == 'flagrunner' && gd[dot].live == true) {
                    ...
                    var targetaquired = x == g.flags2[1].x && y == g.flags2[1].y;
                    var targetoffset = x - 7 == g.flags2[1].x && y - 7 == g.flags2[1].y;
        ...
        moveAttackers1: function(gamename) {
            ...
            if (targetarray.length > 0) {
                for (var i = 0; i < attackers.length; i++) {
                    ...
                    var target = m.nearestTarget(element, targetarray, 'attacker1');
                    if (m.checkCollision(element, target, 'attackers1') == false) {
                        ...
                        io.in(gamename).emit('circlecollision', {'c1': element, 'c2': target, 'host': true});
                    }// end else
                }
            }
        ...
        moveAttackers2: function(gamename) {
            ...
            if (targetarray.length > 0) {
                for (var i = 0; i < attackers.length; i++) {
                    var element = attackers[i];
                    var target = m.nearestTarget(element, targetarray);
                    if (m.checkCollision(element, target, 'attacker2') == false) {
                        ...
                        io.in(gamename).emit('circlecollision', {'c1': element, 'c2': target, 'host': false});
                    }// end else
                }
            }
        ...
        checkCollision: function(circ1, circ2, tag) {
            ...
            var test = distance < circ1.r + circ2.r;
            return test;
        },
