    ...
    methods: {
        ...
        */
        ...
        step: function() {
            ...
            for (let dot in mvd) {
                if (mvd[dot].dottype == 'attacker' && mvd[dot].live == true) { //c(mvd[dot].live + '-' + mvd[dot].dottype + ' -at-test')
                    mm.drawCircle(mvd[dot].x, mvd[dot].y, mvd[dot].r, mv.colors.blue);
                }
                else if (mvd[dot].dottype == 'flagrunner' && mvd[dot].live == true) { //c(mvd[dot].live + '-' + mvd[dot].dottype + ' -fr-test')
...
socket.on('gamestatus', function(data) {
    ...
    if (data.stepcounter > (main.gamestate[main.gamestate.length - 1].stepcounter + 100)) {
        ...
        if (mv.host == true) { // if it's the host client
        ...
        else { // if it's NOT the host client
...
socket.on('circlecollision', function(data) {
    ...
    if (data.host == true) { // if the host client attacker initiated the collision
        if (mv.host == true) { // and if the receiving client is the host
            for (let dot in mv.dots) { // then turn c1 ( the attacker ) .live to false
                if (mv.dots[dot].n == data.c1.n) {
                    mv.dots[dot].live = false;
                }
            }
            for (let dot in mv.edots) { // and c2 ( the target ) .live to false
                if (mv.edots[dot].n == data.c2.n) {
                    mv.edots[dot].live = false;
                }
            }
        }
        else { // else if the receiving client is NOT the host, inversion is necessary
            for (let dot in mv.dots) { // then turn c2 ( the target ) .live to false
                if (mv.dots[dot].n == data.c2.n) {
                    mv.dots[dot].live = false;
                }
            }
            for (let dot in mv.edots) { // and c2 ( the attacker ) .live to false
    ...
    else { // else if the non-host client attacker initiated the collision
        if (mv.host == true) { // and if the receiving client is the host, inversion is necessary
            for (let dot in mv.dots) { // then turn c1 ( the attacker ) .live to false
                if (mv.dots[dot].n == data.c2.n) {
                    mv.dots[dot].live = false;
                }
...
$(main.variables.canvas).mousemove(function(e) { //captures the mouse coordinates within the canvas...
    ...
    $('#mousecoords').html("Mouse Coordinates: " + mv.mouse.x + ' - ' + mv.mouse.y);
...
$('#joingame_wrapper').submit(function(e) {
    ...
    if ($('#gamelist').val() != 'default' && main.variables.username != '') {
        if ($('#gname_info').html() != '') {
            var r = confirm("Are you sure you wish to leave your existing game: " + main.variables.gameroom);
            if (r) {
                socket.emit('joingame', { 'gamename': $('#gamelist').val(), 'oldgamename': main.variables.gameroom, 'username': main.variables.username, 'flags': main.variables.flags });
                $('#gamestatus').html('Game Status: <span class="neongreen_txt">Live</span>');
                //playerturn = 2;            
                main.variables.gamelive = true;
            }
            ...
            socket.emit('joingame', { 'gamename': $('#gamelist').val(), 'username': main.variables.username, 'flags': main.variables.flags });
            $('#gamestatus').html('Game Status: <span class="neongreen_txt">Live</span>');
            //playerturn = 2;
            main.variables.gamelive = true;
        }
