...
const hexF = () => {
  ...
  this.hex_rotate_left = a => {
    return Hex(-a.s, -a.q, -a.r);
  };

  this.hex_rotate_right = a => {
  ...
  this.hex_direction = direction => {
    return hex_directions[direction];
  };

  this.hex_neighbor = (hex, direction) => {
  ...
  this.hex_diagonal_neighbor = (hex, direction) => {
  ...
  this.OffsetCoord = (col, row) => {
    return { col: col, row: row };
  };

  this.EVEN = 1;
  this.ODD = -1;
  this.qoffset_from_cube = (offset, h) => {
    var col = h.q;
    var row = h.r + (h.q + offset * (h.q & 1)) / 2;
    return OffsetCoord(col, row);
  };

  this.qoffset_to_cube = (offset, h) => {
    var q = h.col;
  ...
  this.roffset_from_cube = (offset, h) => {
    var col = h.q + (h.r + offset * (h.r & 1)) / 2;
    var row = h.r;
    return OffsetCoord(col, row);
  };

  this.roffset_to_cube = (offset, h) => {
  ...
  this.Orientation = (f0, f1, f2, f3, b0, b1, b2, b3, start_angle) => {
    return {
      f0: f0,
      f1: f1,
      f2: f2,
  ...
  this.Layout = (orientation, size, origin) => {
    return { orientation: orientation, size: size, origin: origin };
  };

  this.layout_pointy = this.Orientation(
    Math.sqrt(3.0),
    Math.sqrt(3.0) / 2.0,
    0.0,
    3.0 / 2.0,
  ...
  this.layout_flat = this.Orientation(
    3.0 / 2.0,
    0.0,
    Math.sqrt(3.0) / 2.0,
    Math.sqrt(3.0),
  ...
  this.hex_corner_offset = (layout, corner) => {
    var M = layout.orientation;
    var size = layout.size;
...
let F = hexF();
let L = F.Layout(hexF.layout_flat, 20, { x: 200, y: 200 });
let H = {
  q: 0,
  r: 0,
  s: 0
...
console.log(hexF().hex_to_pixel(L, H));
// Tests
